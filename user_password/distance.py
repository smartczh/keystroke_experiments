...
train_rate = 0.5
filter_product = 5
mainData_fileName = "main_data.pk"
...
# 置信区间范围
def calculate_score3(index, sample_type, distance):
    accept_num = 0
    for column in range(0, len(all_data[user][sample_type]["pr"])):
        value = all_data[user][sample_type]["pr"][column][index]
        local_mean = main_data[user]["pr"]["mean"][column]
...
# 生成参数范围供测试
distance_range = [n / 100 for n in range(50, 301)]
# 测试输出结果主体部分
for distance in distance_range:
    ...
    for user in main_data.keys():  # 遍历每个用户
        ...
        for index in range(genuine_test_begin, genuine_test_end):
            total_score = calculate_score3(index, "genuine", distance)
        ...
        for index in range(0, impostor_test_end):
            total_score = calculate_score3(index, "impostor", distance)
        ...
        #     fpr =/negative_num
        #     tpr = /positive_num
    print("eer=" + str(sum(eer_array) / len(eer_array)) + " distance=" + str(distance) + " auc=" + str(
        sum(auc_array) / len(auc_array)))
