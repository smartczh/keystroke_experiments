...
userNum = 124  # 拿多少用户测试，一共有75,todo 改动的话要把文件删了
all_data = {}  # all_data[userid]["genuine/imposter"]["pr/pp"]=[[],[],[]]
allData_fileName = "python_data/all_data.pk"
    ...
    for number in range(1, userNum + 1):
        ...
        if os.path.exists(captures_path):  # 被我过滤了几个user10，18，而且他妈的009genuine是空的草SB
            with open(captures_path) as capture:
                ...
                if len(capture_content) < 20:
                    print(user_path + " is filtered cause imposter not enough")  # 一些傻逼东西他妈的就没负样本
                    continue
        captures_path = root_path + user_path + "genuine/captures.txt"
        if os.path.exists(captures_path):  # 被我过滤了几个user10，18，而且他妈的009genuine是空的草SB
            with open(captures_path) as capture:
                capture_content = capture.readlines()
                if len(capture_content) < 40:
                    print(user_path + " is filtered cause genuine not enough")
                    continue
                all_data[user] = {}
...
train_rate = 0.7
filter_product = 3
mainData_fileName = "python_data/main_data.pk"
...
def calculate_score(index, sample_type, a, b):
    similarity = 0
    pr_size = len(all_data[user][sample_type]["pr"])
    pp_size = len(all_data[user][sample_type]["pp"])
    for column in range(0, pr_size):
        ...
        similarity = similarity + a * math.log(1 + score) / pr_size
    for column in range(0, pp_size):
        ...
        score = np.exp(-(value - local_mean) ** 2 / (2 * local_std ** 2)) / (
                math.sqrt(2 * math.pi) * local_std)
        similarity = similarity + b * math.log(1 + score) / pp_size
    return similarity
...
for user in main_data.keys():  # 遍历每个用户
    ...
    for index in range(genuine_test_begin, genuine_test_end):
        total_score = calculate_score(index, "genuine", 0.3, 0.7)
        positive_score_array.append(total_score)
    if len(all_data[user]["impostor"]["pr"]) == 0 or len(all_data[user]["impostor"]["pp"]) == 0:
        print("sailinmu ")
    ...
    for index in range(0, impostor_test_end):
        total_score = calculate_score(index, "impostor", 0.3, 0.7)
    ...
    #     tpr = /positive_num
eer_array.sort()
n, bins, patches = plt.hist(eer_array, bins=40, color='blue', alpha=0.5, rwidth=0.9, normed=0)  # 直方图
plt.xlabel('EER')
plt.ylabel('frequency')
#plt.title('histogram')
plt.show()
print("eer=" + str(sum(eer_array) / len(eer_array)) + " auc=" + str(sum(auc_array) / len(auc_array)))  # 不需要再除以size
